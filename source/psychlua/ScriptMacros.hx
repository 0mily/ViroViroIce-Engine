...
class ScriptMacros
{
	...
	public static macro function luaFile(pathExpr:ExprOf<String>):ExprOf<String>
	{
		var fileName:String = switch (pathExpr.expr)
		{
			...
			default:
				Context.error('ScriptMacros.luaFile expects a string literal.', pathExpr.pos);
				'';
		};

		var path:String = Context.resolvePath('psychlua/lua/$fileName.lua');
		var content:String = File.getContent(path);
		if (content.length > 0 && content.charCodeAt(0) == 0xFEFF) // this is genuinely hard for me.
