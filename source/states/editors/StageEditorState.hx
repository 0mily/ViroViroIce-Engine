...
class StageEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function showOutput(txt:String, isError:Bool = false)
	{
		...
		{
			if(isError) FlxG.sound.play(Paths.uiSound('cancelMenu'), 0.4);
			else FlxG.sound.play(Paths.uiSound('scrollMenu'), 0.4);
		}
	}
	...
	function addMetaTab()
	{
		...
		var characterList = Character.appendCharacterFileList(Mods.mergeAllTextsNamed('data/characterList.txt'));
