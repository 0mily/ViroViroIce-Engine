...
class Hitbox extends MobileInputManager
{
	...
	var bolasGordas:String = ClientPrefs.data.vsliceControls ? "Hidden" : ClientPrefs.data.hitboxType; // eu me sinto assim: https://tenor.com/view/anais-the-amazing-world-of-gumball-gumball-and-darwin-stantwitter-tweak-gif-738788760275822883 -Shiho
	...
	public function new(?extraMode:ExtraActions = NONE, colorMap:Array<FlxColor>)
	{
		...
		if (!ClientPrefs.data.vsliceControls)
		{
			switch (extraMode)
			{
				case NONE:
	...
	private function createHint(X:Float, Y:Float, Width:Float, Height:Float, Color:Int = 0xFFFFFF):TouchButton
	{
		...
		hint.labelStatusDiff = (bolasGordas != "Hidden") ? ClientPrefs.data.controlsAlpha : 0.00001;
